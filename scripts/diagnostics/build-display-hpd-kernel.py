...
def main():
    ...
    hpd_source = source / 'drivers/gpu/drm/display/drm_bridge_connector.c'
    config = objects / '.config'
    certificate = objects / 'certs/signing_key.x509'
    release_file = objects / 'include/config/kernel.release'
    release = release_file.read_text().strip()
    require(release == '7.2.0-rc3-dirty', 'Unexpected kernel release')
    require('dpu_crtc_needs_dspp' in (source / 'drivers/gpu/drm/msm/disp/dpu1/dpu_crtc.c').read_text(),
            'Installed DPU fix is missing from the matching source')
    require('drm_bridge_connector_should_send_hotplug' not in hpd_source.read_text(),
            'HPD patch is already staged')
    args.output.mkdir(parents=True, exist_ok=False)
    baseline = args.output / 'baseline'
    baseline.mkdir(mode=0o700)
    for path in (hpd_source, objects / 'arch/arm64/boot/Image', config,
                 objects / 'vmlinux.symvers', objects / 'Module.symvers', certificate,
                 release_file):
    ...
    require(release_file.read_text().strip() == release, 'Kernel release changed')
    ...
    metadata = {'image': digest(image), 'kernel_release': release,
                ...
                'patch': digest(patch), 'source': digest(hpd_source)}
