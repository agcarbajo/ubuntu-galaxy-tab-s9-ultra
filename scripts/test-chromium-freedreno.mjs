...
const child=spawn(process.env.CHROME_BINARY || '/usr/bin/google-chrome',['--app=file://'+root+'/probe.html','--ozone-platform=x11','--no-first-run','--no-default-browser-check','--disable-background-networking','--remote-debugging-port=0','--user-data-dir='+profile,'--window-size=1000,900'],{env,stdio:['ignore','ignore','pipe']});
