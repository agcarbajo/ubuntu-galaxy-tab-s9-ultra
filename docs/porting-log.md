...
## Intermittent deep-suspend failure reproduced with the folio

Date: 2026-09-23. The owner reports a black screen after opening the cover, a short power-key press without visible response, and then a spontaneous reboot. The prior boot's persistent system journal was initially lost under the installed 256 MiB `SystemMaxUse` limit; a root-owned diagnostic override on the tablet now sets `SystemMaxUse=1G` and `SyncIntervalSec=30s`, without changing sleep policy. Its original packaged setting is unchanged. This diagnostic setting needs explicit review or removal once investigation ends.

The subsequent reproduction on boot ID `72da1ac306fc4f1c8fae88d5036a5946` is captured. At 18:31:53 CEST the digitizer and logind reported lid closed; `PM: suspend entry (deep)` followed at 18:31:55. At 18:34:11 the kernel logged resume completion and the digitizer/logind reported lid open. The owner confirms this first open and the next close were intentional and that the first resume worked. At 18:34:14 the digitizer/logind reported lid closed and requested a second suspend. The last prior-boot kernel line is `PM: suspend entry (deep)` at 18:34:15, without a subsequent exit or orderly poweroff in that boot. The next boot, ID `43161d6bf0884196b6b25416b5593adc`, recovered the root ext4 journal and deleted four orphans, then mounted the root read/write. `/sys/fs/pstore` was empty. The new boot initially had an incorrect July RTC date, so cross-boot wall-clock timestamps cannot establish exact latency or reset cause.

The first resume printed repeated `geni_i2c a8c000.i2c: error turning SE resources:-13`, but it completed and the owner saw it work. The live device tree identifies `a8c000.i2c` as bus 12 with `digitizer@56`, the Wacom cover reporter. In the pinned GENI I2C source, this error is the negative return from `pm_runtime_get_sync()` during an I2C transfer. Early-resume Qualcomm I2C patches exist upstream, but an association between those transient errors and the subsequent missing resume has not been established; blindly allowing an I2C transfer with disabled runtime PM is not a justified fix. Upstream reviewers explicitly rejected treating `pm_runtime_get_sync() == -EACCES` as success because the controller's clocks/resources may never have been enabled; the July 2026 v7 patch is not safe to import as-is. Charging was observed at 5 V/3 A in the prior boot; the last preceding USB-PD line at 18:29:56 reported 0 mV/0 mA. Neither the I2C errors nor connecting a charger has been proved causal. The running kernel reports `7.2.0-rc3-dirty #2` (build timestamp 2026-07-12); historical #10/#11 test results must not be assigned to this boot without verifying its actual image and source. No kernel, partition, sleep policy or graphical service was changed. The unresolved question is whether the second transaction entered firmware suspend and failed to wake, reset there, or failed during the subsequent resume. Further physical reproduction on the writable root risks another unclean shutdown; any kernel experiment requires the controlled RAM-root/recovery path described in `docs/ramroot-diagnostic.md` and separate authorization for partition writes/reboot.

The owner subsequently authorised writing `init_boot` for a diagnostic, not a kernel or other partition. The exact running `init_boot` and saved Ubuntu copy both hash to `add0ee44a386db86a9860ae6bfe846d3b9424695d05271e93e574fff21c806bd`. The old RAM-root diagnostic is incompatible with that image and the available WSL object trees do not match the running kernel configuration (`/proc/config.gz` SHA-256 `67930b1997348bca2d8f1210667fa58b848872f2b1611bf29a84ac4969aaa651`), so no old module or image was reused. A first-stage preflight-only init_boot was packed from the exact installed image, retaining every original archive entry except the added early init hook, and verified as Android header v4, legacy LZ4, 8 MiB capacity and AVB hash footer. It does not suspend: it mounts Ubuntu read-only without journal replay, verifies and copies the original saved init_boot into RAM, unmounts UFS, restores init_boot with synchronous write and complete readback, and only then requests a return boot. The candidate SHA-256 is `bb7f8447ea3edb675414dc767fb6e24e357af18073a5dbeacc1acc0d0b21081d`. ARM64 SHA-256 ran against the ramdisk's actual loader and libraries under QEMU; simulated success, missing backup, wrong backup and corrupt readback passed. The original partition, root-only on-tablet backup and staged candidate hashes were compared successfully. Battery was 63%, root writable with 397 GiB free, and updater status `complete`. No partition has been written and no reboot was requested yet. Failure before restoration intentionally stops in RAM instead of booting with a half-restored init_boot; manual TWRP recovery would be required in that case. The later actual suspend test and the root cause remain pending.

With explicit approval for writing `init_boot` and two preflight reboots, the diagnostic partition was written with synchronous completion and read back as `bb7f8447…0b21081d`; `boot` stayed `fa3f9e86…25fae7d`. After the first reboot SSH never returned. The owner found a black screen and entered TWRP. Recovery identified the device from `androidboot.em.model=SM-X910` (TWRP itself calls the generic build SM-X710), found the diagnostic hash still in `init_boot` and the original `boot` hash unchanged. TWRP's pstore was empty; `last_kmsg` ended in bootloader output without a diagnostic Linux failure. Thus the one-shot script did **not** reach its restoration/readback; its mocked lifecycle and AVB validation did not prove it would boot on hardware. The actual cause of the early black screen is unproven. An initial hypothesis that the injected hook ran before UFS enumeration is contradicted by the recovered normal boot: its journal places UFS partition enumeration at 11.462 s, before `/init` at 13.677 s. The early failure remains unlocated without a diagnostic kernel trace. Original and candidate AVB footers identify avbtool 1.2.0 and 1.3.0 respectively, but no bootloader rejection was logged.

With fresh explicit approval for recovery, an independently hashed copy of the original image was pushed into TWRP RAM and written **only** to `/dev/block/by-name/init_boot`; its full readback SHA-256 was `add0ee44…6bd`. TWRP reported 61% battery and the unchanged original `boot` hash. The owner-authorized recovery reboot returned to Ubuntu on boot ID `b77ea92277a74a6b9aad30bd7be4832f`: model, original live and saved `init_boot`, original `boot`, writable root, GDM, NetworkManager and Wi-Fi/SSH all checked. The owner physically confirmed the visible Ubuntu login and logged in. No deep suspend test was attempted. The failed diagnostic image must not be reflashed; root cause of the separate suspend failure remains open.

A subsequent **offline-only** candidate (`515324e5240a051ed94db8ed0756ebf0db224639eb249c365655d794eb701960`) moves its hook to after the ordinary initramfs `/scripts/init-premount` path, removes a redundant `/run` tmpfs mount, and runs the helper as a child rather than replacing PID 1. Before any partition write, failure now unmounts the read-only root and lets the original init continue to Ubuntu; failure after a write begins still stops for recovery rather than risking an unverified reboot. CPIO was independently listed by GNU cpio, ARM64 SHA-256 was executed with the bundled original loader/libraries, and simulated success/missing or wrong backup/corrupt readback passed. The original `init_boot` remains installed. These changes address identified weaknesses in the *diagnostic*, not the tablet's suspend problem, and offline checks still do not prove this new image can boot. With a new explicit authorisation for this candidate's `init_boot` write and two preflight reboots, the partition was written and read back as `515324e5…eb701960`, while `boot` remained unchanged. The reboot again resulted in a black screen with no SSH, and the owner entered TWRP. TWRP identified the SM-X910, confirmed the diagnostic still occupied `init_boot`, the original `boot` was unchanged, and pstore was empty; the revised helper did not restore the image either. Both the early-hook and late-hook variants share the re-packed image and altered AVB footer. A common boot/packaging failure is a hypothesis, not a confirmed cause; the absence of a Linux trace cannot distinguish a bootloader rejection from early init failure. Do not flash either diagnostic again.

The owner separately authorised recovery of this second incident. TWRP battery was 74%; the original image was transferred to RAM, hash-verified, written only to `init_boot` with fsync, and read back completely as `add0ee44…6bd`; `boot` retained `fa3f9e86…25fae7d`. After the authorised reboot, Ubuntu boot ID `5b93a3c6-5ac9-474f-a735-18d45b73c33b` returned over SSH: model confirmed, root `rw,noatime`, GDM and NetworkManager active, and live and saved `init_boot` hashes identical to the original. The owner confirmed the visible Ubuntu login and signed in after this second recovery. A later read-only feasibility check found `/swapfile` with 0 B in use, 8 GiB zram with 0 B in use, 12.9 GiB available RAM, deep PM selected and the WINPE microSD mounted writable. `scripts/diagnostics/readonly-suspend.sh` is a source-only staged alternative that stops GDM, disables the UFS swapfile, remounts the root read-only, then waits for supervised lid cycles instead of flashing any boot image. Its mock lifecycle passed normal preparation/storage read/restore and failed-remount rollback; shellcheck and shell syntax passed. The physical tablet's read-only preflight correctly refused to run with the writable WINPE microSD still mounted, and post-check GDM, swap, root rw and microSD were unchanged. The script was copied to root-owned `/run` and must be invoked via `sh` because `/run` is `noexec`; no graphical service was stopped and no sleep was requested. With the owner's authorisation for closing GDM and for a supervised read-only-root suspend, the WINPE microSD was synced and unmounted without force. The live preflight then passed. A detached systemd preparation stopped GDM and disabled the unused UFS swapfile but `mount -o remount,ro /` returned `EBUSY` before any suspend; its rollback restored GDM and the swapfile, leaving root `rw,noatime`. The WINPE volume was remounted at its original path with the original vfat options and ownership. A read-only aggregate of writable file descriptors on the restored root found writers including journald, rsyslogd, tailscaled, CUPS, snapd and other background services; stopping GDM alone cannot quiesce this root. Stopping system services and SSH blindly would remove the only live control path, so no additional remount attempts or suspensions were made. The owner physically confirmed the visible login and logged in after GDM was restored. The root-read-only approach is blocked without a more isolated boot/test environment. A subsequent read-only reset-cause review found that the installed PMK8350 `qcom-pon` driver only writes reboot modes; it does not report the previous power-on/reset reason. The live `/sys/bus/nvmem/devices` is empty, pstore has no records, and the journal of the failing boot ends at the second `PM: suspend entry (deep)`, with no kernel panic, suspend exit or orderly shutdown. The next boot recovered ext4 orphans, establishing an unclean interruption but not whether it was PMIC power loss, firmware watchdog, panic-with-reset, or a later hard reset. Historical TWRP `/proc/last_kmsg` contains ABL `BootReason` and PON flags, but the TWRP captures after the separate diagnostic-image failures contained only old bootloader records and cannot be retroactively assigned to this suspend incident. The first successful wake's Wacom GENI I2C `-EACCES` is a real early-resume error, but the owner confirmed that wake worked; it does not establish why the subsequent deep suspend did not return. No further suspend or reboot was provoked for this review. The original suspend/resume failure remains unresolved.

On 2026-09-24 the owner reproduced a specific lid/charger sequence without an induced test command: cover closed at 00:19, charger attached about ten seconds later, then cover opened and Power tapped; after roughly twenty seconds the owner saw a reboot near 00:23. The prior boot ID `5b93a3c65ac9474fa73518d45b73c33b` records Wacom `book cover closed` at monotonic 12295.219398 (00:19:05 CEST), logind `Lid closed`/`Suspending...`, and `PM: suspend entry (deep)` at 12296.420867 (00:19:06). It has no suspend exit, charger attach, power-key wake, panic or orderly shutdown after that point. The new boot ID `7617bc2a63dd44f3b6b5ed31554d2c72` required ext4 journal recovery and deleted seven orphan inodes; root returned `rw,noatime`. Its early panel platform PM test completed and must not be mistaken for the failed real deep cycle. Pstore remains empty. The PC's PM trace and journal mirrors stopped receiving output when Wi-Fi was taken down for suspension; they could not observe a CPU stuck asleep or a hardware reset. PM debug/tracefs settings reset to their initial off state on the new boot; the existing 1 GiB persistent journal override remains in place.

A source and live-state check found `sm5714_usbpd.connection_wakeup=N`, while `sm5714-usbpd.c` only calls `device_init_wakeup()`/`enable_irq_wake()` when that parameter is true even though the board DTS declares `wakeup-source`. The live USB-PD device consequently exposes no `power/wakeup` control; Wacom's equivalent is `enabled`. This establishes that charger connection is **not an armed USB-PD system wake source** on this build and is consistent with the missing attach log while asleep. It does not establish whether the firmware/PMIC later reset, or whether lid/power IRQ wake failed for another reason; enabling USB-PD wake is a candidate to test, not a demonstrated reboot fix. The post-reboot system exposes no `/sys/class/watchdog/` devices and systemd `RuntimeWatchdogUSec=0`; Linux was booted with `panic=10`, so a kernel panic could cause a delayed reboot, but no panic was recorded and a firmware/PMIC reset remains possible. No partition, GDM, power policy or userspace service was changed in this reproduction. No further partition writes or reboots are implied by standing permission; any next diagnostic needs an observable early-boot failure mode and its own preflight.

## S Pen hover and button-gesture overlap

Date: 2026-09-24. The owner reports that side-button clicks with the S Pen cursor visible can also trigger configured gestures. Source baseline: the hardware service translated both Wacom `BTN_STYLUS` and BLE Button State into gesture mappings without consulting Wacom `BTN_TOOL_PEN`; the kernel publishes that proximity key on entry and synthesises its release on exit. This overlap is reported by the owner, not independently captured on the tablet in this session. Companion 1.4.2 tracks proximity through the existing ungrabbed evdev reader, seeds state via `EVIOCGKEY` on attach, cancels pending/partial gestures on entry and suppresses presses and movement until hover ends, with a release required for an already-held button. The pointer-mode click/drag path is unchanged. The focused synthetic event regression first failed because no proximity handler existed and passed after the change. `python3 scripts/test-companion-efficiency.py`, `test-keyboard-capture.py` and `test-notification-haptics.py` passed in WSL Ubuntu-24.04, as did Python syntax and `git diff --check`. The isolated Debian build validated the schema, Desktop Entry and AppStream; `ubuntu-gts9u-companion_1.4.2_all.deb` has SHA-256 `7d49679229e7e28aa3ae14df2079d31049dd2b031f5e49202d6f25165942314b`, architecture `all`, and a packaged backend byte-for-byte equal to the source (`986556c0ee95f0b70ea458b64bc9676c0eb359b3dfb60a5dc504a943074d6653`). Rootfs builds invoke the package builder and updates copy the selected local Debian package. The owner confirmed the live SSH ED25519 host fingerprint before any credentials were sent. The remote hostname was checked and `/proc/device-tree/model` identified the Samsung Galaxy Tab S9 Ultra Wi-Fi; `hostnamectl` identified Ubuntu 24.04.5 arm64. Before installation, Companion 1.4.1 was active, the single-press mapping was `play-pause`, battery was 73%, 396 GiB was free, root was `rw,noatime` and the update state was `complete`. SFTP placed only the 1.4.2 package in a private temporary directory. The tablet measured the same full package SHA-256. APT simulated exactly one upgrade with zero removals, and `dpkg --audit` was clean. APT installed 1.4.2 with `--no-remove` and unauthenticated archive packages disallowed. The package post-installation restarted its pairing service; the user hardware service was restarted separately to load the new backend. Both services are active, the installed backend hashes exactly to the source/package, and the same isolated regression passed on the tablet against symlinks to the installed backend and pairing service. `dpkg --audit` is clean, root remains writable, and no update is pending. No partition, GDM session or boot image was touched, and no reboot was requested. The owner then physically confirmed that a button press with the digitizer cursor visible no longer triggered the configured `play-pause` gesture, that the same mapping still worked after moving the S Pen far enough for the cursor to disappear, and that Pointer mode still generated a click near the display. The trial left Pointer mode selected; the original Gestures setting was restored over SSH and its value read back. BLE movement suppression while hovering and persistence after a future reboot have not been physically tested. No reboot, partition write or GDM restart was authorized or attempted. No partition backup was needed for this package-only update; the previous 1.4.1 package/hash is recorded in the preceding S Pen entry and 1.4.2's installed package hash is recorded here for package recovery.

## Four-speaker stereo routing and Android quality reference

Date: 2026-09-24. The owner reported that in portrait orientation with the charging port down the left speakers sound louder than the right and that Ubuntu's sound quality is below Android's. Before switching systems, read-only ADB identified One UI 8 / Android 16 on SM-X910 and the live four CS35L45 devices. Samsung's selected mixer initialisation places Front Left/Right/Rear Left/Right on separate TDM slots 0/1/2/3, routes DACPCM from DSP_TX1, and enables DSP Protection when the speaker path is active. Its speaker backend is 48 kHz, four-channel, 24-bit TDM. The four Android boot partitions matched their saved set's complete hashes. Android's source files were inspected only in ignored `work/`; no stock firmware or device-specific calibration was added to Git.

The owner-authorized Dualboot app switched to Ubuntu only after the Android evidence was captured. Its saved Ubuntu images matched the known-good boot hash (`fa3f9e86c5080ab3373077b8a573115c752f54b9c2746a67da1eb92ed25fae7d`), had Android v4 headers and AVB footers, and were checked against partition capacities; the Android backup was hash-matched before the switch. Ubuntu returned with the previously confirmed SSH identity, SM-X910 device-tree model, writable root, active GDM and a complete updater state. No diagnostic kernel or boot image was deployed.

Measured Ubuntu baseline: PipeWire exposed only stereo 16-bit 48 kHz; opening the PCM as four-channel 16-bit or four-channel 24-bit failed. The four amplifier digital volumes were equally 420, and all four DACPCM controls selected ASP_RX1. The exact pinned codec driver and all four live regmaps showed ASP_RX1 is slot 0 and ASP_RX2 is slot 1 (`ASP_FRAME_CONTROL5=0x00000100` for every amp). Thus the right input channel never reached any DAC; adjusting left/right gain would mask, not fix, the routing fault. A focused source regression first failed on Front Right, then passed after Device 2.56 selected ASP_RX2 for both right amplifiers while retaining ASP_RX1 on the left and the same safe gain. The live diagnostic changed only the two right DAC source controls. Regmap readback measured the left amplifier sources at `0x08` and both right sources at `0x09`; one second of silent stereo PCM playback succeeded.

For sound-quality parity, Android's Cirrus DSP image, configuration and generic calibration files were verified byte-for-byte identical to those staged on Ubuntu. A bounded Front Left DSP preload succeeded at loading the Protection program but exposed VSC=0, ISC=0, CAL_STATUS=0 and a VIMON state different from Android's individually nonzero calibration; two mixer-control name collisions were also logged. The preload was switched off without feeding speaker audio through the DSP. No uncalibrated protection or louder amplifier gain was enabled, and no acoustic parity with Android is claimed.

The isolated `ubuntu-gts9u-device_2.56_arm64.deb` is SHA-256 `cdbb4fa06fe1f51e35362477f9d15b9360710c55d0e920b940129496189aba46`. It contains the exact UCM source (SHA-256 `70cc24e16a9e20eeba1f28dc5811581d46447c3d893f73ad9ef0d22f737e8a68`), the same secure owner and identical signed IRQ module as the tablet; the matching kernel object-tree config and certificate were checked. Before upgrade, battery was 78%, root writable with 396 GiB free, no pending update, and `dpkg --audit` clean. The original UCM was backed up root-only at `/var/lib/gts9u-diagnostics/audio-stereo-20260924/alsa-ucm-before.conf` with SHA-256 `2e0362dca448b192a2081a388989bf221bfd89a05bb60629156684a1ea7cdc23`. The transferred Debian hash matched and APT simulated one upgrade, zero removals; installation used `--no-remove` and disallowed unauthenticated archive packages. Afterwards Device 2.56, both right ASP_RX2 controls, DSP preload off, writable root, active GDM/camera relays/ADSP and a clean dpkg audit were verified. The new UCM is packaged for fresh installs and updates; existing live controls were applied diagnostically before installation. No Ubuntu reboot or GDM restart was performed. The owner still needs to compare left/right balance and overall quality audibly and verify persistence after a future reboot.

## Stereo speakers lost again after an ALSA state restore

Date: 2026-09-24. The owner initially heard an improvement on Device 2.56 but reported that after reboot the right channel was silent through all four speakers and some speakers still sounded louder. The verified Ubuntu/SM-X910 SSH identity booted on a new boot ID. The installed 2.56 UCM file still matched the package hash, and all four digital hardware volumes remained equal at 420. Nevertheless Front/Rear Right DACPCM had reverted from ASP_RX2 to ASP_RX1, exactly the pre-fix value. `/var/lib/alsa/asound.state` contained ASP_RX1 for those controls; `alsa-restore.service` ran at monotonic 38.1–38.5 s and restored that stale card state. The subsequent desktop-user WirePlumber restart selected HiFi, but 2.56's UCM Speaker EnableSequence only switched on the amplifiers. Reproducing this after the owner reboot isolated a real persistence failure, not a failed package transfer or an unequal hardware gain.

Device 2.57 keeps the safe BootSequence and also reapplies all four identical digital/analog gains and the per-channel DAC sources in Speaker EnableSequence, before enabling the amplifiers. `ubuntu-gts9u-desktop-user.service` now Wants/After `alsa-restore.service` so its existing one-time WirePlumber/HiFi refresh runs after the saved state is restored. The focused regression first failed because the speaker sequence contained no gains or DAC sources, then passed. The package build reused the exact matching kernel object-tree config and signing certificate and carries the installed secure owner and IRQ module hashes unchanged. `ubuntu-gts9u-device_2.57_arm64.deb` SHA-256 is `0afc6041acdf185a5720e236074446c711a3d29c400300279016ce2e2b66e11a`; its HiFi UCM and desktop-user unit match the tracked source SHA-256 `1887f59a3a77536daf7025f013b16d83933bbca249a6b31bd89821c99a7cea44` and `28db7999a0b6e3d9a0878a698504c47ba9253d1e446a3995fed334d4116a8b12` respectively. The original 2.56 UCM/unit and saved ALSA state were backed up root-only alongside the prior audio backup at `/var/lib/gts9u-diagnostics/audio-stereo-20260924/`; the state backup SHA-256 was `18a20a4b4b713e7047c0c271b3a2998a5deace452acbf13586e26d46b8eb46cd`.

Battery was 51%, root `rw`, update state `complete` and package manager clean before upgrade. The transferred package hash matched and APT simulated one upgrade and zero removals. APT installed 2.57 with removals and unauthenticated archive packages disallowed; the installed UCM/unit hashes matched, and `systemd-analyze verify` succeeded on the tablet. The right controls were still ASP_RX1 immediately after package installation. Restarting WirePlumber alone selected the Speaker device from the new UCM and restored both to ASP_RX2, with PipeWire active and the DSP preload off. A saved-state replay (`alsactl restore 0`) then put both back at ASP_RX1, and a second WirePlumber restart restored ASP_RX2 again. This simulates the measured startup ordering but does not prove a real post-2.57 reboot or even acoustic equality. The stale ALSA state was intentionally not overwritten, so the owner can test the real boot path; GDM, partitions, kernel and boot images were not changed. All four live analog volume controls were also equal at 3 (19 dB), so the owner-reported residual per-speaker loudness difference cannot be attributed to an unequal software gain or safely equalized without measurements and valid DSP calibration.

## ALSA state restore races with the desktop-user audio refresh

Date: 2026-09-24. With explicit authorization for a 2.57 reboot, Ubuntu returned on boot ID `cd7db39312544da0a8dc82c40900f116`: verified SSH identity and SM-X910 model, writable root, active GDM/camera/ADSP/ALSA services and both right DAC sources at ASP_RX2. This was not a conclusive stale-state boot test: `alsa-restore` had saved ASP_RX2 during the prior orderly shutdown. More importantly, the new journal revealed that the Wants/After relationship was insufficient: `alsa-restore.service` was first skipped at 19.5 s because the card did not exist; desktop-user *started* at 33.7 s, a later ALSA restore ran at 39.2–39.5 s, and the WirePlumber/HiFi refresh finished at 40.9 s. On a differently scheduled boot that later ALSA restore could still undo UCM after the refresh.

Device 2.58 therefore makes the existing desktop-user helper wait for `/dev/snd/controlC0`, synchronously start `alsa-restore.service` (harmless if already active) and only then start the user manager and refresh WirePlumber. Speaker EnableSequence still applies matched gains and stereo DAC sources before enabling the amps. The focused regression first failed for the missing synchronization and passed once added; shell syntax, ShellCheck and `git diff --check` passed. The isolated `ubuntu-gts9u-device_2.58_arm64.deb` SHA-256 is `1f0a41681eeaf120c8d8c921b65bd923ddfa96968c44ee71e02165b6b9f7edf3`; the packaged helper matches the source SHA-256 `dd4d0f10324ae3db0c4ff4780e3c431892a17a06d53af83f78ab7b91ab45c0a3`. The packaged kernel module and secure owner remained byte-for-byte identical to the installed copies. The tablet had 48% battery, writable root and a complete update state; transfer hash matched and APT simulated exactly one upgrade, zero removals. APT installed 2.58 with removals and unauthenticated archive packages disallowed; dpkg audit and GDM/camera/ALSA state stayed healthy.

The owner separately authorized a second reboot for 2.58. Ubuntu returned on boot ID `ed2d8381adb84486bb88d3fabc1f05d1`, with the same verified host key and model, Device 2.58 installed, Front/Rear Right ASP_RX2, four equal digital gains of 420, DSP preload off, writable root, active GDM/camera/ADSP/ALSA and the DMIC capture endpoints present. Its journal records an early no-card ALSA restore skip at 19.26 s, a later restore completing at 39.67 s, and the desktop-user HiFi refresh starting afterward at 39.71 s and completing at 41.19 s. This verifies the post-reboot routing and ordering, **not** that a stale ASP_RX1 state survived shutdown: an orderly shutdown had saved ASP_RX2 again. The separate controlled `alsactl restore 0` (ASP_RX1) followed by WirePlumber/UCM reload (ASP_RX2) remains the direct stale-state regression. The owner still needs to listen in portrait with charging port down and report left/right channel audibility, per-speaker balance and remaining quality limitations; no factory calibration or DSP protection was modified.

## Fingerprint compensation transition still visible

Date: 2026-09-24. The owner reports that the earlier fingerprint flash has become a brief darkening of the rest of the panel before and after recognition, roughly half a second by eye. This is an owner observation, not a timed photometric measurement. The source-level reproduction found that Shell sets the full black compensation opacity on `prepare`, then waits for an internal-panel paint and a 35 ms scanout allowance before libfprint is allowed to enable HBM; after the DDIC reports HBM off, the shade remains fully opaque for a 50 ms safety hold. The panel provides only global HBM, so removing the pre-HBM shade or acknowledging before it is painted risks restoring the bright flash. No change to the kernel, capture, templates, PAM or secure owner was justified.

Device 2.59 / overlay 17 changes only Shell presentation: a 50 ms eased entry must complete and be painted before the existing acknowledgement/scanout gate can proceed; the existing 50 ms post-HBM hold is an eased release rather than full-strength black. Rapid retries cancel an old release, unexpected HBM re-entry restores compensation immediately, and cleanup invalidates outstanding animation callbacks. The regression initially failed because the previous implementation applied the full shade immediately; the mock now covers pre-ack opacity, painting only after the ease, exit, rapid retry, re-entry, brightness polling during a fade and cancellation. The focused Shell and visual-state Node tests pass on WSL Ubuntu-24.04 (with `--experimental-default-type=module`), as do 20 broker tests on Linux, JS syntax and `git diff --check`. The Windows broker run cannot execute the existing `os.fchmod` call; Linux's complete pass resolves that platform-only error. The owner explicitly authorized rebuilding the Device package, including replacement of its existing WSL staging directory. The WSL Ubuntu build script checked the v1.2.0 kernel object's configuration, signing certificate and release against the packaged SPSS module. `ubuntu-gts9u-device_2.59_arm64.deb` has SHA-256 `2efd0be64846e29b442fa021dccaefcbd2aae62bee6538039663ef50971daf5c`; staged overlay JS SHA-256 `1e61ab1b6418c5f4953c13c2c9d57b23afef6ee0b1541ec5ac469c9edc31ba72` and metadata `d9218daba935b5bf193c109f53434fdf59b2df0c3112aa58cc764784f338a0f5` match source. Architecture is arm64 and the libfprint minimum remains gts9u48. The rootfs build selects the latest built Device package into `out/local-debs`; the updater copies that selected package, but a clean rootfs build remains unperformed; the initial absence of tablet checks was superseded by the deployment below. The owner confirmed the public ED25519 host-key fingerprint announced by the tablet before any authentication. Batch-mode SSH from both development environments failed with `Permission denied (publickey,password)`; password-only access was not used through logged agent tools. At this first checkpoint, tablet identity beyond the key and physical validation were still pending; the owner later opened a password-authenticated master connection from their own terminal, as recorded below. A new GNOME login is required after installation to load the changed ES module. At minimum, compare lock/unlock and GDM login visually at low/high brightness and through quick repeated scans, release/cancel and rotation; a mock pass alone does not establish that the rest of the OLED stays visually constant.

The owner then provided current SSH details and independently confirmed the announced ED25519 host key. A password-authenticated master connection was opened by the owner in their own terminal; no password was put into a command, file, project log or test output. Read-only checks identified the same Ubuntu 24.04.5 arm64 boot and Samsung Galaxy Tab S9 Ultra Wi-Fi model, Device 2.58 and libfprint gts9u48, writable root, 292 GiB free, 31% battery and update state `complete`; `dpkg --audit` was empty. The 2.59 package transferred to `/tmp` read back with the full build SHA-256 `2efd0be64846e29b442fa021dccaefcbd2aae62bee6538039663ef50971daf5c`. APT simulation selected exactly one upgrade, zero new packages and zero removals. Before installation the installed overlay JS and metadata had SHA-256 `e96c7053e379b147e5fad6140fb0b0eaa9652ae776f9db4b0748cdaf801605c1` and `83091f02a83609683529bd3aeaf5dc421eca20d70b4125fee12eecd05a010551`; their hash-verified copies are retained root-only under `/root/ubuntu-gts9u/work/fingerprint-overlay-2.58-{extension.js,metadata.json}` in WSL `Ubuntu` for recovery. No previous 2.58 `.deb` was present in the checked package cache.

The owner entered the sudo password only in their own terminal and installed the local 2.59 package with `--no-remove`, unauthenticated archive packages disabled and conffiles preserved. APT reported one upgrade and zero removals. Afterward the same host key/hostname, model and boot ID remained; `dpkg-query` reported Device 2.59 and libfprint gts9u48; installed overlay JS and metadata matched source SHA-256 `1e61ab1b6418c5f4953c13c2c9d57b23afef6ee0b1541ec5ac469c9edc31ba72` and `d9218daba935b5bf193c109f53434fdf59b2df0c3112aa58cc764784f338a0f5`. `dpkg --audit` remained empty, root remained `rw,noatime`, update state `complete`, and GDM, the fingerprint UI, boot-only secure owner and camera relays were active; battery was 32%. The owner authorized a GDM restart to load extension 17, but it has not yet occurred: remote `sudo -n` requires a fresh interactive password. No boot partition, kernel, fingerprint templates or Android image was modified, and no visual acceptance has yet been claimed.

After the separately approved GDM restart, the active local Shell reported overlay diagnostics version 17. The owner physically confirmed that the rest of the screen **still darkens briefly at both edges**, but has normal brightness while the optical target remains lit. This refutes the hypothesis that easing the shade alone would make the transition invisible; the package, extension and input path were working, not stale. The read-only `--observe` option added to `scripts/diagnostics/test-fingerprint-presentation.py` sampled only the root-owned light request/acknowledgement and the panel HBM state; it read no images, templates or biometric results and wrote no panel mode. In the owner-triggered lock-screen capture, `prepare` appeared at observer t=17,955.0 ms, `presented` at 18,070.1 (+115.1 ms), and sysfs `fod_mode=1` at 18,138.7 (+68.6 ms). HBM stayed on until 21,031.2 (about 2.89 s), the request disappeared 10.5 ms later and the active visual lease disappeared another 41.4 ms later. Sysfs reports mode after its locked write and 35 ms settle, so the first light-emitting frame may precede the observer's `fod_mode=1` sample; these timestamps do not measure emitted luminance. The 2.59 Shell animation itself adds 50 ms before acknowledgement, and Shell and libfprint still poll at 50/45 ms respectively. Reducing these bounded waits is the next source-level candidate. The existing standalone GJS stage-signal probe could not run on the tablet (`Clutter` typelib 14 is not installed outside GNOME Shell); upstream signal documentation is not proof that replacing the scanout gate is safe on this GNOME 46 build. No further service or partition change was made during this diagnosis.

## Fingerprint edges: paired timing candidate, not yet on hardware

Date: 2026-09-25 CEST. After the owner confirmed the dip is confined to entry and exit rather than the ~2.89 s HBM interval, a smaller source-only candidate replaces the ineffective 50 ms entry/exit ease with 20 ms easing and watches the root-owned `light` and `active` directory entries only while an optical operation is active. This wakes the active GNOME Shell promptly for a new request and for release, without raising its idle 50 ms sysfs polling rate. Any file-monitor failure retains that periodic fallback. The existing stage after-paint, internal-panel check, full-compensation gate and 35 ms scanout allowance are unchanged, so a missing or stale broker response still aborts HBM rather than revealing an unmasked frame. Libfprint polls the broker acknowledgement every 5 ms only while its unique pending light token exists; all other capture, timeout, cancel and idle paths keep their original 45 ms poll. Device 2.60 requires libfprint gts9u49, and the latter Breaks older Device packages to force a paired APT transaction; no kernel or secure owner code changed. `build-ubuntu-rootfs.sh` hashes changed libfprint source/build inputs and rebuilds it, while its local Debian selection includes both packages for clean images and updates.

The Shell mock first failed on the prior 50 ms fade; the native C test initially failed because the pending-only poll helper was absent. With the candidate, eight GLib light-state tests (compiled natively on WSL Ubuntu-24.04), the Shell state/paint/monitor/fallback tests, visual lease states and 20 broker tests pass. JS syntax, Python diagnostic compilation, builder shell syntax and `git diff --check` pass. A WSL `/tmp` test binary disappeared after that distribution stopped; recompiling to its persistent Linux `out/` and repeating all eight tests passed. These are simulated timing and source behavior, **not** a physical improvement measurement. No gts9u49/Device 2.60 `.deb` has been built or installed, no tablet APT simulation or new hashes exist, and the owner has not seen this candidate. The installed device remains 2.59 with gts9u48; the verified previous overlay backup remains in WSL `Ubuntu` as recorded above. Both package builders clear existing staging with `rm -rf`; without explicit approval for each replacement, they were not run while the owner slept. There was no further tablet service restart, partition write, reboot or optical test in this step. A trusted paired build, APT simulation, authorized installation/new GNOME session and owner-visible comparison at low/high brightness are still required; do not mark the dark dip resolved on mock results.

After the owner specifically authorized replacing WSL staging at `/root/ubuntu-gts9u/buildroot/build/el721-libfprint`, `/root/ubuntu-gts9u/build/deb/libfprint-2-2` and `/root/ubuntu-gts9u/build/deb/ubuntu-gts9u-device`, `scripts/build-libfprint-el721.sh` rebuilt the exact pinned libfprint, quic-teec and QCBOR sources in its existing arm64 Noble chroot. All its focused UI lease, touch-to-light, gallery, print, retry, IdentifyDo, EnrollDo, HwVault, QTEE and SPL tests passed; the real `drivers_el721.c` compiled and linked into `libfprint-2.so.2.0.0`. The three existing source caches were verified as valid bare repositories, so no mirror cache was discarded. The buildroot's resolver content was already byte-for-byte identical to WSL's, and the build script restored the same bytes. The arm64 `libfprint-2-2_1.94.7+tod1-0ubuntu5~24.04.8+gts9u49_arm64.deb` hashes to `556d229086a423fc2e85bfc14b12fcf483ad84e4fd18298d7543042455d2a2db`; it Breaks Device versions below 2.60. The matching kernel object-tree configuration, module signing certificate and release checks in `scripts/build-device-package.sh` passed again; `ubuntu-gts9u-device_2.60_arm64.deb` hashes to `7aa021de8a21e5c2157c6ff1c90c4f0331623c1b696ad4db4e4ee75caf6ade85` and Depends on libfprint gts9u49 or newer. The staged Shell JS `d7f1c2fd51eb04851652cc0e4c95614120efb714c1909100c3fcd1c375fcf334`, metadata `d1ed38b342d146f51f7576d76e00bab4fa44029e7e45961c9767e08e99db8ac8`, and patched C driver/header `d5fa038b39c0a72be36790a89f381ae3250035146da510102b7ca2f6f09ee44d` / `c01c4f4cc4d08620de83180f83a286246c8b163216a3889f381c7ee93773efc3` match their tracked inputs. These are package/build facts, not device or visual validation.

After an owner-reported tablet reboot, mDNS on the development PC resolved its previously observed hostname to a different LAN address, whose SSH ED25519 host key independently matched the owner-confirmed fingerprint from the prior session. The former SSH master is no longer available; no credentials have been sent to the new address, and hostname, model and Ubuntu boot identity have **not yet** been rechecked there. The paired packages have not been transferred, APT-simulated or installed on this new boot; root rw, battery, free space and update status must be read again before doing so. No boot partition, kernel, GDM or other tablet service was changed by these WSL package builds. Extracting both finished `.deb`s independently confirmed the packaged overlay JS and metadata match the source hashes above, the packaged libfprint shared object matches the stripped arm64 build (`b0e0b356aa807b02542f76e52c7f156ff9642d6ce078d747bdf933341d91f2cd`), and the packaged SPSS IRQ module and boot-lifetime secure owner match their corresponding build artifacts (`ce393f9ce8b639bda8cf27261250da006a6fe93844346efcc528edb8f55411a2` and `b3c58ba477e22dda720632769def396f4470a1476cd5212f1ff2aaa9ac15eb2a`). Visual acceptance remains pending.

Date: 2026-09-25 CEST. The owner supplied current SSH details and asked to finish deploying the candidate. The SSH ED25519 key matched the previously recorded tablet key before credentials were sent; the logged-in host identified as Samsung Galaxy Tab S9 Ultra Wi-Fi on Ubuntu 24.04.5 arm64, boot ID `af4773b7-f466-42f0-bd38-44b626ddc8e6`. Device 2.59 and libfprint gts9u48 were still installed. The root was writable with 287 GiB free, battery 19% and charging, update state `complete`, and `dpkg --audit` was empty. Both WSL packages retained their recorded SHA-256 values (`556d2290…2d2db` and `7aa021de…e85`); transfer to `/tmp` yielded identical full hashes. APT simulation selected exactly two upgrades, zero new packages and zero removals. APT installed both with removals disallowed and archive unauthenticated packages disabled. The post-install versions are Device 2.60 and libfprint gts9u49; overlay JS, metadata and libfprint shared object match their recorded source/build hashes. The UI broker, camera relays and GDM were active, root remained writable and `dpkg --audit` was empty. The running user Shell still reported overlay 17 immediately after APT, as expected for GNOME 46 ES modules.

The owner explicitly authorized a GDM restart to load the new extension. GDM returned with an active Wayland greeter, and its private session bus `GetDiagnostics` reported overlay version 18 with `greeter=true`. The UI broker and camera relays stayed active, battery rose to 20% while charging, and root remained writable. No boot partition, kernel, secure owner, fingerprint template or Android image was changed. The owner still needs to test actual login/unlock and Companion recognition at low/high brightness, rapid retries, cancellation and rotation, watching for a returning flash or residual dark dip. Package installation and greeter loading do not establish optical improvement.

The owner then tested the new session and reported that the pre/post-capture darkening looks essentially the same, possibly a little shorter. The active user Shell's `GetDiagnostics` reported overlay 18, so this is not a stale module. A read-only 10 ms observer of two owner-triggered verifications measured, in the first attempt, `prepare` at t=56,761.8 ms, broker `presented` at 56,835.8 (+74.0 ms), and sampled `fod_mode=1` at 56,881.0 (+45.2 ms). HBM returned to zero at 59,904.7, the request cleared 10.2 ms later and the active lease 40.7 ms after that. The second attempt measured `prepare` at 11,557.0 ms and sampled `presented` and HBM at 11,668.0 (+111.0 ms); after HBM fell at 13,016.8, the request cleared in 10.3 ms and the lease in another 51.4 ms. This observer does not measure the start/end of the actual emitted-light transition, and simultaneous samples do not prove simultaneous physical presentation. The panel driver exposes global FlatZ/HBM only, while GNOME's `after-paint` is before display presentation; removing the frame/scanout gate or shade would risk returning the original full-screen flash. No further code or tablet change was made. The 2.60 timing candidate is physically **not accepted** as a fix for the dark edges.

Date: 2026-09-26 CEST. The owner supplied `20260926_004556.mp4`, recorded with `com.android.capture.fps=240` and encoded near 30 fps for slow playback. The original was read without modification. One owner-triggered `sudo su` fingerprint attempt appears in the tablet journal at 00:45:57.553 (contact/request), 00:45:57.871 (capture settled) and 00:45:58.349 (match result). The video shows a fixed background area away from the finger and terminal falling from paired-frame camera Y about 88 to 49 for frames 224–231, then recovering. At exit the same crop falls from about 98 to 80 over frames 402–415 and rebounds to about 130 near frames 420–421, then settles. A separate terminal crop follows the same direction, while an off-tablet reference crop stays near Y 35–38. The illuminated finger target remains visible through about frame 427, overlapping the bright rebound. With consecutive 240 fps frames, the entry dip spans approximately 33 ms, the exit dip 58 ms and the rebound 83 ms; these are camera-encoded values and approximate optical intervals, not calibrated luminance or a frame-perfect alignment with the journal. The video reveals an exit bright rebound as well as the reported dark dip. The repository panel driver's `fod_mode=0` path sends normal-mode and brightness DCS commands, waits 35 ms, then reports the state change; Shell begins its 20 ms shade release once it observes HBM off. Before attributing behavior to that driver, verify the running image contains this exact code. The video alone cannot assign the rebound to one command or prove that changing the shade timing would improve both edges. No tablet or source code change was made from this video analysis.

Date: 2026-09-26 CEST. A paired exit-transition candidate is installed for owner testing, without a kernel or partition change. Libfprint gts9u50 publishes an expiring `release` marker before its blocking `fod_mode=0` write. Device 2.61 / overlay 19 begins a 55 ms shade release on that marker instead of waiting until the panel sysfs mode reports zero after its settle; if the marker is missed, the previous post-HBM release remains, and a failed panel write removes the marker so Shell restores full shade if HBM remains on. The protected entry and its 35 ms scanout allowance are unchanged. This specifically tests whether overlapping shade withdrawal with the panel exit reduces the measured exit dip; it cannot be claimed to remove the video rebound before a new 240 fps comparison. The Shell release/rearm mock and visual-state tests passed; Python diagnostic compilation, shell syntax and `git diff --check` passed. The exact pinned Noble arm64 libfprint source compiled and its focused native tests passed. The signed IRQ module and kernel object-tree certificate/config/release checks passed using `/root/ubuntu-gts9u/build/linux-release-1.2.0` and `/root/ubuntu-gts9u/out/kernel-gts9uwifi`; an initial default-tree build correctly refused a signing-certificate mismatch. Device package SHA-256 `8649ed49c132772b33d9342297b0feef59be03519dfad00850cdc572be7870da`; libfprint package SHA-256 `b534654f73e7066028fd5606bc445264b5fe1a5dd9877820fc92cfb48168e6f9`. The packaged extension and visual-state files matched tracked source, the packaged libfprint library matched the arm64 staged library, and the unchanged module matched the prior installed package. The tablet retained the previously verified SSH identity and SM-X910 model; before install battery was 70%, root was writable and update state `complete`. The transferred package hashes matched exactly. APT simulated two upgrades, zero new packages and zero removals; installation completed with removals disabled and unauthenticated archive packages disallowed. Installed versions and extension/library hashes match the packages; GDM, fingerprint UI, boot-only secure owner and camera relays are active, and `dpkg --audit` is empty. The active GNOME session still needs to be replaced to load overlay 19, so no physical improvement has been claimed. The owner has been asked separately for the GDM restart required by `AGENTS.md`; no restart has occurred in this step.

The owner explicitly approved the new GDM restart. It returned to an active Wayland greeter with the same tablet boot and all four checked services active. The greeter's private session-bus `GetDiagnostics` reported overlay version 19, `greeter=true`, `sessionActive=true`, and the target/shade hidden while idle. This verifies that the new JS is loaded, not that its optical timing is improved. The owner has been asked to compare physical login and unlock with 2.60, watching for residual darkening and any new bright flash. No partition write or reboot was made.

Date: 2026-09-26 CEST. The owner supplied a second 240 fps slow-motion recording after loading overlay 19 and reported that it looks almost the same, though the overall period since the initial version may feel shorter. The active user Shell's private-bus diagnostics confirmed version 19. The video has 1138 encoded frames and the same 1920×1080 geometry as the earlier recording. In the same fixed background crop, paired-frame Y is about 99 before exit, falls to about 78 by frames 898–899, rebounds to about 134 at 904–905, and settles near 84 by 928; the terminal follows the same direction while an off-tablet crop stays around 22. Entry falls from about 97 to 50 across frames 230–239. This is a camera measurement with no calibrated nits or cross-video exposure control, but it does not show a clear elimination of either optical edge. The repository panel driver holds `ctx->lock` across its DCS off commands and 35 ms settle, and `fod_mode_show` takes the same lock. Overlay 19 synchronously reads `fod_mode` before looking for its short release hint; this is a concrete source-level race that can defer the Shell main loop until libfprint has removed the hint. The running kernel's exact source is not independently attested here, so this remains a proposed explanation rather than a measured panel trace. A GNOME GJS probe on the tablet confirmed that `Gio.File.load_contents_async` works for this sysfs attribute. Device 2.62 / overlay 20 uses one outstanding asynchronous mode read and gts9u51 keeps the separate `last-release` marker until a new contact, with one-second parser expiry. The focused mock first reproduced the pending-read condition and now verifies the hint is processed during it. Physical comparison is still required.

The paired 2.62/gts9u51 source candidate then built from the pinned Noble arm64 sources in WSL. Native libfprint component tests passed and the actual `drivers_el721.c` compiled into the shared library. The Shell mock verifies one outstanding asynchronous sysfs read and a release hint processed before that read completes; it also passes the existing entry presentation, exit/rearm, file-monitor and cleanup cases. JS syntax, diagnostic Python compilation and `git diff --check` passed. The Device builder verified the matched released-kernel config, signing certificate, module signature and kernel release; the unchanged IRQ module hash remained `ce393f9c…11a2`. Packaged Shell source matched the tracked extension. Device 2.62 SHA-256 is `e5d0de7848d6a5c949b4f460d55bee7020c6df3b3f03a446fb5a839748a9b024`; libfprint gts9u51 SHA-256 is `298db52e9fa29599f7afd552e1a03027e8a14175f4e46b215c9b5265b5aafdf0`; the packaged shared library matched the arm64 staged library hash `890a39246791329f4dbaa7a4ca3f1d7f2f4ad67ecdb1960c649a6ce31139c913`. On the same verified tablet, battery was 62%, root writable, updater complete and package audit clean. Transferred package hashes matched. APT simulation selected exactly the paired upgrades, zero new packages and zero removals. Installation completed with removals disabled and unauthenticated archive packages disallowed. Installed versions, extension files and shared-library hashes match the build. GDM, fingerprint UI, boot-only secure owner and camera relays remain active; `dpkg --audit` is empty. The current Shell still has overlay 19 in memory. The owner has been asked for the separately required GDM restart authorization before overlay 20 can be tested. No partition or boot image was written, and no reboot occurred.

The owner explicitly authorised that GDM restart. `systemctl restart gdm3` returned successfully without a tablet reboot; `gdm3` is active and a new `gdm` GNOME Shell owns the Wayland greeter. Its private session-bus `GetDiagnostics` reports `version:20`, `greeter:true`, `sessionActive:true`, `releaseHintsSeen:0` and idle target/shade. The installed libfprint version remains `1:1.94.7+tod1-0ubuntu5~24.04.8+gts9u51`, and `dpkg --audit` remains empty. A read-only 60-second observer started immediately after the owner's test invitation saw only idle state (`active=0`, `prepare=0`, `release=0`, `hbm=0`); no fingerprint attempt occurred inside that window, so it provides no timing or visual result. The owner has been invited to perform the physical comparison. Optical improvement remains unverified.

The owner supplied a third 240 fps slow-motion recording (`20260926_021116.mp4`) after testing overlay 20 and reports that the darkening is still visible. The new user Shell is running `version:20` and reports `releaseHintsSeen:2`, with its most recent release hint processed after 1 ms. A `sudo su` fingerprint verification appears in the tablet journal at 02:11:18.426 (contact), 02:11:18.739 (capture settled) and 02:11:20.019 (first result), followed by a second candidate capture/result. In the same tablet-background crop, the camera level falls from around 48 to 11–20 across frames 214–221 before reaching an HBM plateau of around 54–59. At exit it dips near frames 935–942, rebounds to roughly 79–84 at 949–955, and drops again around frame 968. The 1354 decoded source frames declare 240 fps capture and are encoded near 30 fps for slow playback. The relative frame intervals suggest about 33 ms of entry dip and 80–100 ms of exit rebound, but camera exposure alternates and the image is not calibrated to panel luminance. Overlay 20 therefore fixes the software hint delivery but does not fix the visible symptom. Panel and compositor state need a synchronized trace before changing the release curve or DCS sequence again. No further package, kernel or partition change was made from this third video alone.

On a coordinated real `sudo su` attempt, the read-only runtime observer saw `prepare` at 71,366.1 ms, its painted-frame acknowledgement and `fod_mode=1` at 71,471.1 ms, and the release marker and `fod_mode=0` together at 72,631.3 ms; the request cleared 10.8 ms later and the active lease 42.1 ms after that. Shell reported three release hints total, the latest again after 1 ms. The observer reads `fod_mode` synchronously, however, so a read can block on the panel mutex and make marker and mode appear simultaneous even if they were not. A separate temporary kernel kprobe trace on a second owner-triggered attempt measured `ana38407_write_fod_locked` entry at monotonic 22069.494282 with `enable=1` and return at 22069.534425 (40.143 ms); the `enable=0` path ran from 22074.523403 to 22074.566800 (43.397 ms), both successful. The tracefs probes were then disabled and removed; `kprobe_events` is empty. These timings locate the software DDIC transaction but do not measure the panel's emitted light or establish exactly which optical frame corresponds to the DCS commands. No kernel or package change was made.

The owner then compared low and high display brightness and reported that the darkening remains perceptible at both and scales with the chosen brightness. This is consistent with the full-screen black compensation preceding global HBM, but is not a calibrated measurement of its opacity. The active Mutter display configuration is 2960×1848 at 120 Hz, with 60 and 30 Hz modes available. On the tablet, GObject lists `ClutterStage::presented` with a `ClutterStageView` and a raw `gpointer` argument; GNOME 46's source marks the signal skipped for introspection, and GJS 1.80's conversion code rejects non-null untyped pointer arguments. A standalone GJS probe could look up the native signal, but that does not make its frame-info argument usable by the Shell extension. No presentation bridge or compositor modification was installed; shortening the 35 ms scanout allowance without a reliable presented-frame acknowledgement could expose global HBM before the shade is visible. The exit dip/rebound is a separate remaining issue. The installed packages stay at Device 2.62 and libfprint gts9u51.

A small native presentation bridge was then built as an offline candidate and installed with Device 2.63 for later owner testing. Its C GObject receives `ClutterStage::presented`, drops the opaque frame-info pointer, and emits only the introspectable view; Shell requires two matching internal-panel presentations after its full shade paint before acknowledging libfprint. The previous 35 ms acknowledgement timer remains as a fallback. New read-only counters distinguish native and fallback gates and report the latest gate duration. This changes the entry timing only; the exit curve remains 2.62's and is still visibly flawed. The arm64 bridge compiled against Noble's Mutter 14 headers, generated a GIR/typelib, and imported successfully through the live tablet's GJS 1.80.2 from `/tmp`, without altering the active Shell. Its installed library hash is `f67c613b5eef7e0fe8833a0a2b8b472ff98db8084bfeac27c9e3fb3d87e80953` and typelib hash `1ddf7ba935ba5687bdfa334348d3dc769e853f9ec487c8a7b15ebaeaf9c530b8`. The focused Shell mock passes native two-frame gating, 35 ms fallback and cancellation; the visual-state test, JS syntax, shell syntax and `git diff --check` pass. The signed IRQ module and kernel configuration/certificate/release checks again passed in the Device builder. Device 2.63 package SHA-256 is `9feeeb0e80a8aa4376a520b230a5505d69112939404a912a8fbad25aaaacd3a6`. Comparing package file hashes with 2.62 found only the two new bridge files and changed extension/metadata, besides Debian control. Before tablet installation, the same SM-X910 was on Ubuntu 24.04 with writable root, 52% battery, 287 GiB free, no active update and clean `dpkg --audit`. The transfer hash matched. APT simulation selected one upgrade, zero new packages and zero removals. The Device upgrade completed with removals disabled and unauthenticated archives disallowed. Installed bridge hashes and versions match; GDM, fingerprint UI, boot-lifetime secure owner and camera relays are active, and `dpkg --audit` remains clean. The active user Shell still reports overlay 20. `AGENTS.md` requires a new explicit owner approval before restarting GDM to load overlay 21; no restart, reboot or partition write occurred in this step.

The owner then explicitly authorised one GDM restart. It completed, GDM remained active, and the greeter's D-Bus diagnostics reported overlay version 21. After the owner's physical fingerprint attempt, the user Shell reported `nativeGatesUsed:1`, `fallbackGatesUsed:0`, `lastLightGateMs:20` and a release hint delay of 2 ms. The owner reported that the visible darkening still looked the same. Thus the native presentation gate worked, but its 15 ms reduction from the previous fallback did not visibly resolve the symptom. An exit-timing variant was briefly edited locally, but its package build stopped at the IRQ signing-key check before producing a package; those edits were removed at the owner's request to stop. Device 2.63 / overlay 21 remains installed and no further restart or tablet change was made. The fingerprint effect is improved compared with the beginning of this investigation according to the owner, but is still visible; the owner chose to leave it as is.

The owner then changed course and explicitly authorized finishing and deploying that exit variant. The failed build had used the default kernel object tree, whose signing certificate did not match the pinned IRQ module. `/root/ubuntu-gts9u/build/linux-release-1.2.0` had the matching certificate serial, an identical configuration to the released kernel output and the same `7.2.0-rc3-dirty` release. With that tree selected, Device 2.64 built successfully; package SHA-256 is `65fe8f817b701f1df0c8e466178ed1dcc5b4dda01e165c2c12c5fab551867a76`. Compared with 2.63, its installed file payload changes only `extension.js` and `metadata.json`. The extension keeps shade compensation while panel HBM is on, including after the early release hint, then starts a 20 ms fade after the asynchronous mode read observes zero. The existing 50 ms post-off lease remains. Focused Shell and visual-state mocks, JS syntax and `git diff --check` passed. The tablet was verified as the same Galaxy Tab S9 Ultra Wi-Fi on Ubuntu 24.04.5, battery 50%, root writable, 287 GiB free, updater complete and package audit empty. Transfer hash matched and APT simulation selected one upgrade with no new or removed packages. Installation used `--no-remove` and rejected unauthenticated archives. Device 2.64, exact extension hash, active GDM/fingerprint services and a clean package audit were verified. The owner-authorized GDM restart completed and the greeter reported overlay version 22. The owner performed a fingerprint attempt and reported that the darkening still looks the same; no timing/video capture was made for this variant. The owner chose to leave it here. Device 2.64 / overlay 22 remains installed; no further tablet change is planned.

## 2026-09-26: Ubuntu 26.04 upgrade assessment

The owner requested a separate branch for Ubuntu 26.04, GNOME 50 and the latest viable stable kernel, with a non-destructive updater ZIP and no tablet flashing. `upgrade/ubuntu-26.04-gnome50` was created from `main` after checking branch and working-tree state; an unrelated untracked file and the virtualisation worktree were left untouched. Official Ubuntu, GNOME and kernel.org sources identify Resolute 26.04, GNOME 50 and stable Linux 7.2.8. A read-only SSH session, after matching the previously saved ED25519 host-key fingerprint, identified the physical SM-X910 at Ubuntu 24.04.5, `7.2.0-rc3-dirty`, updater state complete at v1.2.0. No tablet write, package operation, reboot or flash occurred.

The current updater and release pipeline are Noble-only: ZIP release matching, manifest suite validation, device preflight, APT sources and custom Mutter/GNOME Settings Daemon/libfprint packages all require migration before a 26.04 package can preserve data safely. Temporary release-build guards now reject mixed rootfs/suite inputs. The detailed dependency and physical validation matrix is in `docs/upgrade-26.04-assessment.md`. An initial kernel-build command unintentionally used the existing shared WSL build directory because its isolation variables were omitted; it was stopped promptly, and no cleanup/reset was attempted to avoid disturbing another agent's generated state. Subsequent 7.2.8 work uses only `/root/ubuntu-gts9u-2604` with its own upstream clone, port-source clone, source worktree, object directory and output. A full 26.04 rootfs, updater migration and usable ZIP have not been produced.

The isolated Linux 7.2.8 build then completed successfully from official stable commit `9a66fdc0d7fd55f54235524a73435af99051e46f`. All staged port patches applied without rejects; `Image.gz` SHA-256 is `c7e1d9799726e4a1fb5d1b9d48c6cca2400b4db539e89ce9e0af6097856b5b53`, and 57 signed modules report `7.2.8-dirty` vermagic. This is a compilation result only. The new DTB (`12998b0a25fc763ab1dc6ea58a91ff488c5808440582cadc5c659ef530d5ab3c`, 177,828 bytes) differs from the previous WSL output by two PCIe `iommu-map` cells and 16 bytes. Since Samsung ABL has rejected structural DTB changes before Linux logging, boot and Wi-Fi compatibility are unproven. The GNOME 50 userspace, offline Noble-to-Resolute migration, boot bundle, update ZIP and physical validation remain outstanding; none was installed or flashed. The two focused release-suite guard tests, shell syntax, Python compilation and `git diff --check` passed.
