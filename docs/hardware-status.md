...
| S Pen: emparejamiento BLE | ❌ | 🟡 | confirmado | El comando Wacom 0xea abre el anuncio al acoplarlo. El servicio aceptó la autorización iniciada por el lápiz y confirmó `Bonded`, `Paired`, `Trusted` sin interacción. Insertado usa siempre Mode `0x10`. Tab Companion valida el GATT real, descarta porcentajes en caché y reemplaza automáticamente un vínculo inservible. Desde 0.10.4 la recuperación post-reinicio está acotada a dos intentos de 12 s y vuelve a abrir el anuncio si vence la primera ventana |
