...
| Fingerprint: EL721/UDFPS infrastructure | ❌ | 🟡 | partial | QTEE 5.2.0, UID 122, GPIO91/GPIO155 power/reset, BAUTH, FlatZ HBM, the GNOME indicator and Goodix-FOD events/suppression are physically validated. One UI 8 confirms that `securefp` is visible to its UID 1000 client and that its BAUTH layout matches the probe; reproducing that client identity on Ubuntu is the next test. The DDIC circle needs Self Display and is not used; details in [fingerprint-reader.md](fingerprint-reader.md) |
