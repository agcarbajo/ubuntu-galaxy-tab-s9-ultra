...
| Fingerprint: EL721/UDFPS infrastructure | ❌ | 🟡 | partial | QTEE 5.2.0, UID 122, GPIO91/GPIO155 power, BAUTH, FlatZ HBM, the GNOME indicator and Goodix-FOD events/suppression are physically validated. Ubuntu now reproduces One UI's signed `dualfp` load, physical DMA32 regions, VMID/perms, two `0x2a4000` buffers and no-reset start sequence, but the TA still reports sensor type zero instead of EL721 type 8. The remaining boundary is secure sensor communication; details in [fingerprint-reader.md](fingerprint-reader.md) |
