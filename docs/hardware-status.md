...
| Adreno 740 GPU | ✅ | ✅ | measured | Freedreno/Turnip works in games. Chrome and ChatGPT now use hardware ANGLE/Vulkan; ChatGPT also selects native Wayland. Discord gradients and Sheets text are corrected, with improved responsiveness confirmed by the owner. See [graphics validation](chromium-vulkan-rendering.md). Other clients' sporadic GTK repaint artefacts remain a separate issue |
...
| Brightness / blanking | ✅ | ✅ | confirmed | Manual and GNOME automatic brightness work. Samsung's SSC `auto_brightness` stream supplies lux; room lighting tests verified dimming and brightening ([evidence](light-sensor-reference.md)). Resume reliability is tracked separately below |
...
| S Pen: gestures and pointer | ❌ | ✅ | confirmed | The BLE single/double/long gestures and the six movements were confirmed before 1.4.2. The pointer enables the raw sensor at ~24 Hz, discards the lengthwise X roll and projects gyro Y/Z with gravity to compensate for the pen's roll. After the visual test both axes were inverted and X uses a 1.6× gain. In Pointer mode, `BTN_STYLUS` produces a held `BTN_LEFT` for click and drag, with a safety release. One capture produced 621 `REL_X/REL_Y` events. With 1.4.2 installed, the owner confirmed no side-button gesture while the Wacom cursor was visible, that the mapped gesture still worked out of range, and that pointer click still worked near the screen. BLE movement during hover and persistence through a reboot remain untested |
