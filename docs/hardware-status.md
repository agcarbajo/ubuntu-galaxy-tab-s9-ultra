...
| Giroscopio LSM6DSO | ✅ | ✅ | medido | El mismo canal SSC. El LSM6DSO cuelga de **SPI** (`bus_type=3`, `bus_instance=1`), que es la única ruta de sensores viva bajo Ubuntu |
| Brújula AK0991x | ✅ | ❌ | medido | **Regresión frente a pmOS.** Bajo Ubuntu `iio-sensor-proxy` 3.9 no publica la interfaz `net.hadess.SensorProxy.Compass`; `gdbus introspect` sólo lista `net.hadess.SensorProxy`. El AK0991x está en I²C (`bus_instance=2`, 0x0c) y **ningún sensor I²C del SSC produce datos en esta build**: sólo funciona el que va por SPI |
...
| Luz ambiental STK31610 | ❌ | ❌ | medido | El registro Samsung sitúa los dos chips en I²C `bus_instance` 3 y 4, esclavo 72 (**0x48**), raíles `dummy_vdd`. Esos dos motores son `i2c_hub_3` (0x98c000) e `i2c_hub_4` (0x990000), que el AP ya conduce en GPI-DMA: un `i2cdetect` completo de ambos devuelve **sólo** 0x63 (SM5440) y 0x18 (MAX77816), y 0x48 hace NAK en los 16 buses del AP. Con el bus demostrado bueno por sus vecinos, **el chip no responde a ninguno de los dos maestros**. Vía AP cerrada por medida, no por falta de intento; la vía SSC sigue sin emitir lux. `ssc-light` no se ofrece para no bloquear GNOME |
