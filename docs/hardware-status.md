...
| S Pen: emparejamiento BLE | ❌ | 🟡 | confirmado | El comando Wacom 0xea abre el anuncio al acoplarlo. El servicio aceptó la autorización iniciada por el lápiz y confirmó `Bonded`, `Paired`, `Trusted` sin interacción. Insertado usa siempre Mode `0x10`. Una sesión fantasma mantenía `Connected=yes` pero rechazaba todo GATT y reproducía batería `00 00` en caché; tras limpiar BlueZ y repetir el flujo, Battery Level devolvió 100 y Mode respondió. 0.10.1 exige salud GATT y repara automáticamente esa condición con el lápiz insertado |
| S Pen: gestos y puntero | ❌ | 🟡 | medido | Los gestos BLE simple/doble/larga y seis movimientos están confirmados. El puntero activa el sensor raw a ~24 Hz, descarta el giro longitudinal X y proyecta gyro Y/Z con la gravedad para compensar la rotación del lápiz. Una captura de extremo a extremo produjo 621 eventos `REL_X/REL_Y` reales en GNOME. Falta cerrar la sensación final del cursor |
...
| Vibración / hápticos | ❌ | ✅ | confirmado | El DTS stock identifica `dc_vibrator` COINDC en TLMM GPIO18 y mainline lo publica como `gpio-vibrator`/`FF_RUMBLE`. La usuaria confirmó motor y teclado en pantalla; Tab Companion ofrece pulsos de 12/22/34 ms. Las notificaciones vibran de forma opcional y una prueba real midió GPIO554 activo durante 64,5 ms |
