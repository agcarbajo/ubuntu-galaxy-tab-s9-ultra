...
Última actualización: 2026-08-12, tras integrar y medir el acoplamiento,
orientación y estado discreto de carga del S Pen con Tab Companion 0.2.0.
...
| Tab Companion | — | 🟡 | medido | Paquete `ubuntu-gts9u-companion` 0.4.0 instalado. Su servicio D-Bus entrega estado real y el motor `uinput` ejecutó una asignación completa hasta `KEY_VOLUMEUP`, observada con `evtest`. La UI permite elegir acción, destino y aprender la siguiente tecla; reconoce simple/doble/larga del botón. Falta confirmación visual en GNOME real y movimiento BLE |
...
| S Pen: acoplamiento y orientación | ❌ | 🟡 | medido | El driver reclama TLMM GPIO137 (`PDCT`), publica `SW_PEN_INSERTED` y atributos sysfs. Con el lápiz físicamente acoplado, punta hacia el USB-C, se midieron `pen_docked=1`, switch a 1 y respuesta Wacom `direction=2`; Tab Companion la calibra como `tip-right`. Falta validar manualmente retirada, reinserción y la orientación contraria |
| S Pen: batería y carga | ❌ | 🟡 | medido | `gts9u-spen` aparece como `power_supply` de ámbito dispositivo con `present=1` y estado discreto `Not charging` medido. El protocolo Samsung ofrece cargar/completo/no cargar pero no porcentaje; se expone `PenBattery=-1` y UPower no lo enumera, deliberadamente, en vez de fabricar capacidad |
| S Pen: emparejamiento BLE | ❌ | ❌ | medido | Escaneo BLE activo de 25 s con el lápiz acoplado: 22 dispositivos visibles, ninguno identificado como S Pen; sólo estaba emparejado un Pro Controller. Como el lápiz no se puso en modo emparejamiento, su ausencia no prueba que el perfil no funcione |
| S Pen: gestos | ❌ | 🟡 | construido | Tab Companion monitoriza `BTN_STYLUS` sin captura exclusiva y distingue simple (ventana 300 ms), doble y larga (600 ms); el autómata produjo las tres asignaciones correctas en prueba software y reutiliza el motor `uinput`. No se pudo pulsar físicamente. Deslizamientos y círculos siguen sin transporte: requieren emparejar y caracterizar el GATT Samsung |
...
| Teclado pogo EF-DX920 (STM32 I²C 0x2a) | ❌ | ✅ | confirmado | Requiere V37 en el MCU: con V34 la aplicación pulsa CONN y no anuncia el protocolo. Reprogramado a V37, teclea desde arranque en frío y sobrevive a desconectar y reconectar la funda. Tab Companion monitoriza el mismo dispositivo sin `EVIOCGRAB` y remapea por `uinput`; Galaxy AI `0x2f8`, DeX `0x2bd` y Search 217 proceden del fuente Samsung. `Fn+F1–F5` siguen sin código hasta pulsarlos físicamente y pueden aprenderse desde la UI |
