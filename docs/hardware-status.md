...
Last updated: 2026-09-02, after validating authenticated HwVault restoration
and integrated EL721 discovery through fprintd on Ubuntu.
...
| Fingerprint: EL721/UDFPS infrastructure | ❌ | 🟡 | measured | QTEE 5.2.0, UID 122, GPIO91/GPIO155 power, BAUTH, FlatZ HBM, the GNOME indicator and Goodix-FOD events/suppression are physically validated. Ubuntu now restores HwVault's authenticated K250A credentials 0/1/11 (353/2,533/185 bytes, all status zero), keeps the derived-key context resident, completes stock-equivalent `Prepare` with every error field zero, and exposes `EgisTec EL721 UDFPS` through fprintd's real systemd sandbox. Details in [fingerprint-reader.md](fingerprint-reader.md) |
| Fingerprint: enrolment, verification and GDM | ❌ | ❌ | measured | Physical capture previously reached 17 accepted samples and 100% coverage; its result-71 encryption blocker is now resolved in the integrated backend. A fresh encrypted enrolment, same/different-finger match, cancellation, lockout and PAM/GDM still require the owner's finger, so biometric authentication is not yet declared operational |
