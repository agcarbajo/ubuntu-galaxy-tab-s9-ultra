...
| Tab Companion | — | 🟡 | medido | Paquete `ubuntu-gts9u-companion` 0.5.0 instalado. Sustituye el desplegable GTK defectuoso por una lista no reciclable, hace explícito «Learn» con cancelación/caducidad y contiene las 12 teclas Fn. El servicio D-Bus entrega estado real y el motor `uinput` aplica sustitución selectiva. Falta confirmación visual final en GNOME real y movimiento BLE |
...
| S Pen: batería y carga | ❌ | 🟡 | confirmado | Al insertar, el kernel envía automáticamente enable/start/keep-on. En un arranque limpio se midió `S Pen charging started` a 4,54 s, respuestas de carga 5 y 8, y `gts9u-spen/status=Charging`. El protocolo del garaje no ofrece porcentaje intermedio; publica 100 % sólo al indicar completo y deja `PenBattery=-1` hasta obtener Battery por BLE |
...
| Teclado pogo EF-DX920 (STM32 I²C 0x2a) | ❌ | ✅ | confirmado | Requiere V37 en el MCU. Se midieron Galaxy AI 760, DeX 701, Finder 710, Ajustes 709 y Fn+F1–F11: 757, 758, 759, 705, 254, 172, 224, 225, 113, 114 y 115. Fn+F12 no genera evento bruto. Tab Companion 0.5.0 captura y retransmite el teclado para poder sustituir sólo las asignaciones activas, conservando por defecto Inicio/brillo/volumen de Fn+F6–F11 |
