...
| S Pen: gestos | ❌ | ❌ | reconocido | GNOME no puede cubrirlo: las acciones de botón de stylus son sólo `default, middle, right, back, forward`, y `keybinding` sólo existe para botones de *pad*. `evtest` confirma `BTN_STYLUS` en `event2`, pero no se pudo pulsar durante la sesión. Requiere demonio propio (uinput), GSettings y app propia; los gestos BLE quedan pendientes del GATT |
