...
| S Pen: gestos y puntero | ❌ | 🟡 | confirmado | Los gestos BLE simple/doble/larga y seis movimientos están confirmados. 0.9.0 permite apagar por completo la conexión remota o elegir un puntero relativo con clic/arrastre, sensibilidad, suavizado y aceleración; las dos funciones nuevas requieren prueba física final |
...
| Vibración / hápticos | ❌ | 🟡 | medido | El DTS stock identifica `dc_vibrator` COINDC en TLMM GPIO18. Mainline lo publica como `gpio-vibrator`/`FF_RUMBLE`; una prueba de 500 ms trazó el GPIO global 554 a 1 y después a 0. Tab Companion añade prueba y realimentación ajustable para el teclado en pantalla. Falta confirmación táctil de la usuaria |
