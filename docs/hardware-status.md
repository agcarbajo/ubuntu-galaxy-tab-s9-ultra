...
| S Pen: acoplamiento (detección de guardado) | ❌ | ❌ | reconocido | El fuente oficial confirma que TLMM GPIO137 (`PDCT`) es la señal *pen in/out* del hueco y que Samsung publica `SW_PEN_INSERT`. La tablet sólo se ha observado con el lápiz dentro; falta integrar el GPIO y validar ambos flancos físicamente |
| S Pen: batería y carga | ❌ | ❌ | reconocido | UPower sólo enumera la batería SM5714 y las dos alimentaciones USB; no hay dispositivo del lápiz. El digitalizador Wacom sí gobierna y notifica estados discretos de carga BLE, pero el fuente oficial no ofrece porcentaje de batería. No se confundirá «cargando/completo» con un nivel inventado |
